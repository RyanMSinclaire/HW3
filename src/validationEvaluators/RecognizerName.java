...
public class RecognizerName { // FSM for evaluating names.
	...
	private static boolean running;							// The flag that specifies if the FSM is 
															// running
	private static int nameSize = 0;						// A numeric value may not exceed 160
															// characters
	...
	public static String checkForValidName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// If the size is larger than 160, the loop must stop
				if (nameSize > 160)
					running = false;
				break;
			...
			case 1: 
				...
				//  2: a hyphen or space that transitions to state 2
				//	3: a period that transitions to state 3
			...
			case 2: 
				// State 2 deals with a character after a hyphen or space in the name
			...
			case 3:
				// State 3 deals with a character after a period
				
				// Space -> State 0
				if (currentChar == ' ') {								// Check for a space
					nextState = 0;
		...
		switch (state) {
		case 0:
			// State 0 is not a final state. Check if this is the initial character
			if (nameSize > 0) {
				nameRecognizerErrorMessage += "Your name must have A-z or a-z after a space.";
			}
			else
				nameRecognizerErrorMessage += "Your name must start with A-Z or a-z.\n";
		...
		case 1:
			...
			if (currentCharNdx < input.length()) {
				...
				nameRecognizerErrorMessage += 
					"Your name may only contain the characters A-Z, a-z, period, or hyphen.\n";
			}
			else if (nameSize > 160) {
				// Name is too long
				nameRecognizerErrorMessage += "Your name must have no more than 160 characters.\n";
			}
			else {
				// Name is valid
		...
		case 2:
			...
			nameRecognizerErrorMessage +=
				"Your name must have A-Z or a-z after a hyphen or space.\n";
			return nameRecognizerErrorMessage;
			
		case 3:
			// State 3 is not a final state, so we can return a very specific error message
			nameRecognizerErrorMessage +=
				"Only a space is valid after a period in your name.\n";
			return nameRecognizerErrorMessage;

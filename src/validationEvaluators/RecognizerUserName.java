...
public class RecognizerUserName { 
	...
	*/
	...
	public static String checkForValidUserName(String input) {
		...
		if(input.length() <= 0) {
			...
			return "\n*** ERROR *** The Username input is empty";
		}
	...
	//	System.out.println("\nCurrent Final Input  Next  Date\nState   State Char  State  Size");
		...
		while (running) {
			...
			case 1: 
				...
				// . - _ -> State 2
				// Added - and _ to the else if statement for State 2
				else if ((currentChar == '.') ||						// Check for .
							(currentChar == '-') ||						// Check for -
							(currentChar == '_')) {						// Check for _
					nextState = 2;
					
					// Count the period, underscore, or minus
					userNameSize++;
				}				
				// If it is none of those characters, the FSM halts
				else
			...
			case 2: 
				// State 2 deals with a character after a period, minus,
		...
		case 1:
			...
			if (currentCharNdx < input.length()) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z, 0-9, period, -, _.\n";
			}
